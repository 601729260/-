...
(select d.order_no,item_id,(case when e.order_state=1 then 0 when e.order_state=2 then 0 else -item_count end) as item_count from u_order_line d inner join u_order e on d.order_no=e.order_no where d.create_time>='2019-01-29 00:00:00' and e.order_type not in(5,4) and d.item_type=3  and e.s_id!=1500000150 union all 
...
where ba.pay_time>='2019-01-29 00:00:00' and e.order_type not in (4,5) and ba.refund_status=0  AND ba.refund_item_type=3 and e.s_id!=1500000150 ) c
