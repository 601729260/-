...
select a.shop_id,a.item_num_id,a.qty,ifnull(c.qty_change,0),ifnull(b.qty,0) bak_qty from t_gb_stock a left join t_gb_stock_20190128 b on a.shop_id=b.shop_id  and a.item_num_id=b.item_num_id
...
select a.storage_id,a.item_num_id,(a.qty+a.p_qty) as total_qty,ifnull(c.total_qty_change,0),ifnull(b.qty+b.p_qty,0) bak_qty from t_gb_stock a left join t_gb_stock_20190128 b on a.storage_id=b.storage_id  and a.item_num_id=b.item_num_id
left join  (select sid,item_num_id,sum(qty_change+p_qty_change) as total_qty_change from u_goods_stock_log c where c.create_time>='2019-01-29 00:00:00'  and sid_type=3 and bill_type!=3 group by sid,item_num_id) c
...
select * from u_goods_stock_log where id='STORAGE_45_胡三妹1548724601999';


select * from u_goods_stock_log  where bill_type=300002;

300001  公共仓和app仓互转
300002 虚拟仓库存调整


select distinct bill_type from u_goods_stock_log ;
...
where c.create_time>='2019-01-29 00:00:00'  
and sid_type=2  
...
(select d.order_no,item_id,(case when e.order_state=1 then 0 when e.order_state=2 then 0 else -item_count end) as item_count from u_order_line d inner join u_order e on d.order_no=e.order_no where d.create_time>='2019-01-29 00:00:00' and e.order_type not in(5,4) and d.item_type=3 union all 
...
where ba.pay_time>='2019-01-29 00:00:00' and e.order_type not in (4,5) and ba.refund_status=0 ) c
...
group by id,sid,item_num_id) aa
right join 
(select order_no ,good_item_id as item_id ,sum(-count) as item_count from u_order_deposit b where b.created_time>='2019-01-29 00:00:00' group by  order_no ,good_item_id) bb
on aa.id=bb.order_no and aa.item_num_id=bb.item_id
where ifnull(aa.qty_change,0)!=bb.item_count
...
select * from u_goods_stock_log where id='145290137178074';
