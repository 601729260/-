...
create table t_average_cost_price_bak20190604 as select * from  t_average_cost_price;
...
create table t_average_cost_price_bak20191022 as select * from  t_average_cost_price;
update t_average_cost_price a left join (
select sid,item_num_id ,sum(purchase_price*(qty_change+p_qty_change)) as price,sum(qty_change+p_qty_change) as num,sum(purchase_price*(qty_change+p_qty_change))/sum(qty_change+p_qty_change) as result 
from u_goods_stock_log 
where bill_type in (0,6)
...
# 校验有问题的采购单
select a.sid as 总部ID,a.item_num_id  as  商品sku ,a.avarage_purchase_price as 平均采购价,b.result as 正确结果 from  t_average_cost_price a left join (
