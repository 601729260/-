...
select * from GB_CONSUMER_TRAN_UNIT_temp;
...
WHERE
	T2.SYNC_BATCH_ID = 201907251733591463;
...
WHERE
	...
	AND T1.SYNC_BATCH_ID = 201907251733591463;
...
WHERE
	SYNC_BATCH_ID = 201907251733591463;
...
WHERE
	T2.SYNC_BATCH_ID IN ( 201903110946115992 );
...
WHERE
	...
	AND T1.SYNC_BATCH_ID IN ( 201903110946115992 );
...
FROM
	GB_STORAGE_O2O_TEMP 
WHERE
	SYNC_BATCH_ID IN ( 201903110946115992);
...
UPDATE GB_CORT_ORN T1
			INNER JOIN GB_CORT_ORN_TEMP T2 ON 
			T2.ORN_NUM_ID=T1.ORN_NUM_ID 
		SET
			T1.ORN_NUM_ID = T2.ORN_NUM_ID, 
			T1.ORN_NAME = T2.ORN_NAME
			where  T2.SYNC_BATCH_ID in (201907251533591435)
...
INSERT INTO GB_CORT_ORN
		...
		FROM GB_CORT_ORN_TEMP T1 WHERE NOT EXISTS(
			SELECT 1 FROM GB_CORT_ORN T2
			 WHERE 
			 T2.ORN_NUM_ID = T1.ORN_NUM_ID
		) and T1.SYNC_BATCH_ID in (201907251533591435)
		
		delete from GB_CORT_ORN_TEMP where SYNC_BATCH_ID in (201907251533591435);
...
INSERT INTO GB_STORAGE_O2O
		...
		FROM GB_STORAGE_O2O_TEMP T1 WHERE NOT EXISTS(
			SELECT 1 FROM GB_STORAGE_O2O T2
			 ...
			 T2.FROM_STORAGE_NUM_ID = T1.FROM_STORAGE_NUM_ID -->
		) and T1.SYNC_BATCH_ID in (201907122004116664);

delete from   GB_STORAGE_O2O_TEMP where SYNC_BATCH_ID in (201907122004116664);
...
delete from  	gb_catelog_pty2_temp;
delete from   gb_catelog_pty3_temp;
		
		

gb_catelog_pty2_temp  2406
gb_catelog_pty3_temp  1
